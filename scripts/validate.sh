...
grep -q 'Start FeatherOS' config/bootloaders/grub-pc/grub.cfg
grep -q 'FeatherOS (Debug Mode)' config/bootloaders/grub-pc/grub.cfg
grep -q 'console=tty0 console=ttyS0,115200' config/bootloaders/grub-pc/grub.cfg
grub_debug_line="$(grep '^[[:space:]]*linux .*debug' config/bootloaders/grub-pc/grub.cfg)"
